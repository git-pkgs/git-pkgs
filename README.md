...
```bash
git pkgs health                 # show maintenance health scores
git pkgs health --threshold=70  # show lower-scoring dependencies only
git pkgs health --all           # include transitive lockfile dependencies
git pkgs health --format=json
```
...
git pkgs changelog express -e npm   # installed version to latest
git pkgs changelog express -e npm --from=  # all entries up to latest
```
...
When `--from` is omitted, a versioned PURL supplies the lower bound; otherwise the command infers the unique resolved version at HEAD. Multiple installed versions produce an error listing their versions and manifests so you can choose `--from` explicitly. If the package has no resolved installed version, or the command runs outside a Git repository, the lower bound remains open. An explicit `--from` always takes precedence; `--from=` requests an open lower bound.
