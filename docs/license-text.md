...
Artifact resolution requires a registry supported by `git-pkgs/registries` and an available package download. Dependencies whose package URL has a `repository_url` qualifier that is not an HTTP or HTTPS address (Bundler `PATH` sources, npm `file:` dependencies, and similar local paths) are skipped and appear in output without archive-derived fields. A lockfile may contain hashes for several platform-specific files while its parsed dependency record retains one hash. In that case the scan covers the selected archive rather than every file published for that version. Registry authentication is not configured by this command.
